...
class CroppedZeroMeanDataProvider(ZeroMeanDataProvider):
    ...
    def __init__(
        ...
        self.cropped_data = [
            np.zeros((self.get_data_dims(), self.data_dic[0]['data'].shape[1] * self.data_mult), dtype=np.single) for x in xrange(2)]
    ...
    def get_next_batch(self):
        epoch, batchnum, (data, labels) = ZeroMeanDataProvider.get_next_batch(self)
    ...
    def get_plottable_data(self, data):
        return np.require((data + self.data_mean).T.reshape(data.shape[1], self.band_count, self.inner_height, self.inner_width).swapaxes(1,3).swapaxes(1,2) / 255.0, dtype=np.single)
